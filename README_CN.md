...
2. 根据原理图里的有标记，购买对应的元器件，然后按照丝印焊接到 PCB 上。关于元器件选择，请看后面的说明。
...
#### 元器件说明
* D1, D2, D3 是红外发射管 IR333C-A;
* Q2, Q4, Q6 是光电晶体管 PT333-3B。
* Q1, Q3, Q5 是低 Rds 的 N 沟 MOSFET，例如 AO3400，见下方计算。
* R2, R4, R6 是限流电阻，见下方计算。
* R1, R3, R5 是 MOSFET 栅极电阻，10ohm 即可。
* R7, R8, R9 是上拉电阻，2Kohm 即可。
* R10 是下拉电阻，10Kohm 即可。

* 以 D1 为例做一下计算，我们需要 1A 左右的电流以获得合适的信号强度。  
  D1 的电流计算公式为 `I = (Vcc - Vf) / (Rds + R2)`。Vcc 为 3.3V，Vf 为 D1 的正向电压（约 2.6V@1A）。对于低 Rds 的 N 沟 MOSFET，例如 AO3400，它的 Rds 约为 0.047ohm，因此 R2 应该约为 0.7ohm，所以可以选择 0.68ohm 的电阻。你可以使用其他较低 Rds 的 N 沟道 MOSFET，但需要重新计算电阻值。
